...
void CMainDlg::DoDataExchange(CDataExchange *pDX) {
    ...
    DDX_Control(pDX, IDC_SAVE_JOBS_BUTTON, m_saveJobsButtonCtl);
    DDX_Control(pDX, IDC_SCHEDULE_JOBS_BUTTON, m_scheduleJobBtnCtl);
...
BEGIN_MESSAGE_MAP(CMainDlg, CInsyncDlg)
    ...
    ON_BN_CLICKED(IDC_SAVE_JOBS_BUTTON, OnSaveJobsButton)
    ON_BN_CLICKED(IDC_SCHEDULE_JOBS_BUTTON, OnBnClickedScheduleJobsButton)
...
BOOL CMainDlg::OnInitDialog() {
    ...
    m_scheduleJobBtnCtl.Init(IDI_SCHEDULE, m_pToolTip,
                             _T("Schedule selected jobs to run at a future time."));
    m_saveJobsButtonCtl.Init(IDI_SAVE, m_pToolTip,
                             _T("Save list of selected jobs to a file. You can later double click on the file, or pass it as a command line parameter to run the selected jobs."));
...
void CMainDlg::RunJobs(bool simulate) {
    ...
    if (count) {
        ...
        m_saveJobsButtonCtl.EnableWindow(FALSE);
        ...
        m_scheduleJobBtnCtl.EnableWindow(FALSE);
...
bool CMainDlg::UpdateJobControls() {
    ...
    m_saveJobsButtonCtl.EnableWindow(count);
    if (m_currentJob) {
...
LRESULT CMainDlg::OnJobsDone(WPARAM, LPARAM) {
    ...
    m_saveJobsButtonCtl.EnableWindow(TRUE);
    ...
    m_scheduleJobBtnCtl.EnableWindow(TRUE);
    return 0;
}
...
void CMainDlg::OnSaveJobsButton() {
    UpdateJobControls();
    LPCTSTR szFilter = _T("InSync Job Selection Files (*.isy)||");
    LPCTSTR nullStr = kNullString;
    HANDLE hndl;
    CJobNode *jobNode;
    int count, indx;
    TCHAR name[255];
    ...
    InSyncMessageBox(true, _T("Warning. This does not actually save the job definitions. It ")
                     _T("only save the names of the currently selected jobs. You can then run ")
                     _T("this selection of jobs by double clicking on the saved job list file"),
                     MB_ICONWARNING | MB_OK);
    CFileDialog dlg(FALSE,
                    kInSyncFileType,
                    _T("UntitledJobSelection"),
                    OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT,
                    szFilter);

    if (dlg.DoModal() == IDOK) {
        if ((hndl = ::CreateFile(dlg.m_ofn.lpstrFile, GENERIC_WRITE, FILE_SHARE_READ, NULL, CREATE_ALWAYS,
                                 FILE_ATTRIBUTE_NORMAL, NULL)) != INVALID_HANDLE_VALUE) {
            TCHAR uheader = 0xfeff;
            ::WriteFile(hndl, &uheader, sizeof(uheader), &written, NULL);
            m_mandalaIx = 0;
            if ((count = m_jobListCtl.GetSelCount()) != 0) {
                int *indxList = new int[count];
                if (indxList) {
                    if (m_jobListCtl.GetSelItems(count, indxList) != LB_ERR) {
                        for (indx = 0; indx < count; indx++) {
                            jobNode = (CJobNode *)m_jobListCtl.GetItemDataPtr(indxList[indx]);
                            _tcscpy_s(name, jobNode->m_name);
                            name[_countof(name) - 1] = 0;
                            ::WriteFile(hndl, name, (DWORD)(_tcslen(name) + 1) * sizeof
                                        (TCHAR), &written, NULL);
                        }
                    }
                    delete [] indxList;
            ...
            ::WriteFile(hndl, nullStr, (DWORD)(_tcslen(nullStr) + 1) * sizeof
                        (TCHAR), &written, NULL);
            ::CloseHandle(hndl);
        }
    }
...
void CMainDlg::OnBnClickedScheduleJobsButton() {
    UpdateJobControls();
    ITaskScheduler *pISched = NULL;
    // Get an interface to the scheduler.
    HRESULT hr = ::CoCreateInstance(CLSID_CTaskScheduler, NULL, CLSCTX_INPROC_SERVER,
                                    IID_ITaskScheduler, (LPVOID *)&pISched);
    if (HRFAILED(hr)) {
        if (REGDB_E_CLASSNOTREG == hr)
            InSyncMessageBox(true, _T("Windows scheduler is not running!"), MB_ICONERROR | MB_OK);
        else {
            InSyncMessageBox(true, _T("The windows scheduler has failed or is not functioning correctly."),
                             MB_ICONERROR | MB_OK);
        }
        return;
    }
