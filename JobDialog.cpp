...
BOOL CJobDialog::OnInitDialog() {
...

    {
        ...
        for (node = CMainDlg::Singleton().m_jobList.begin(); node != CMainDlg::Singleton().m_jobList.end(); ++node) {
            jobNode = *node;
...
void CJobDialog::OnOK() {
    ...
    if (_tcslen(jobname) == 0) {
        CMainDlg::Singleton().InSyncMessageBox(true, _T("Job name must be specified"), MB_ICONERROR | MB_OK);
        return;
    }
    m_jobNode->m_name = jobname;

    if (m_jobNode->m_childrenList.size() == 0) {
        CMainDlg::Singleton().InSyncMessageBox(true, _T("No source folders are specified!"), MB_ICONERROR | MB_OK);
        return;
    }
    ...
    for (snode = m_jobNode->m_childrenList.begin(); snode != m_jobNode->m_childrenList.end(); ++snode) {
        ...
        if (src->m_childrenList.empty()) {
            CMainDlg::Singleton().InSyncMessageBox(true, CString(_T("Source folder ")) + src->m_name + _T(" has no target folder"),
                                                   MB_ICONERROR | MB_OK);
            return;
        }
    }
...
void CJobDialog::UpdateSrcControls() {
    ...
    if (oneSelected) {
        ...
        for (node = m_currentSrc->m_childrenList.begin(); node != m_currentSrc->m_childrenList.end(); ++node) {
            CTrgNode *trg = *node;
...
void CJobDialog::UpdatePostTrgControls() {
    ...
    if (oneSelected) {
        ...
        m_currentTrgIx = indx;
        m_trgDontDeleteCheckCtl.EnableWindow(oneSelected && (m_currentTrg->m_parentJob->m_mode == MIRROR_SYNC_MODE));
