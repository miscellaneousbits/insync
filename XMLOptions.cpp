...
bool CXmlOptions::SaveXMLOptions()
{
    ...
    for (JobNodeListIter jobNode = CMainDlg::Singleton().m_jobList.begin();
         jobNode != CMainDlg::Singleton().m_jobList.end();
         ++jobNode) {
        ...
        if ((*jobNode)->m_runAfterList.size()) {
            ...
            for (ConstStrSetIter runIter = (*jobNode)->m_runAfterList.begin();
                 runIter != (*jobNode)->m_runAfterList.end(); ++runIter) {
                CXmlNode runAfter = runList.NewChild(RUNAFTERKEY);
                runAfter.SetAttribute(_T("Name"), *runIter);
            }
        }

        for (SrcNodeListIter srcNode = (*jobNode)->m_childrenList.begin();
             srcNode != (*jobNode)->m_childrenList.end(); ++srcNode) {
