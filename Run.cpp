...
{
    for (JobNodeListIter node = CMainDlg::Singleton().m_jobList.begin();
         node != CMainDlg::Singleton().m_jobList.end(); ++node) {
...
UINT CRun::CJobsThread()
{
    ...
    if (jobCount) {
        if (!abortFlag) {
            ...
            m_jobsActive = true;
            m_poolBuffer = VirtualAlloc(NULL,
                                        CMainDlg::Singleton().DefaultMaxThreads() * kMaxThreadBufferSize * 2,
...
void CRun::ScheduleDirMerge(FileDataList &fromList, FileDataList &toList, const CFilePath &fromDir,
...
void CRun::ScheduleBackupOrSync(const CFilePath &fromDir, const CFilePath &toDir,
                                const bool fromExists, const bool toExists,
                                CTrgNode *trg)
{
    trg->m_pendingTasks++;
