...
void CJobNode::WriteToLog(LPCTSTR text)
{
    ...
    if ((m_logFile != INVALID_HANDLE_VALUE) &&
        ...
        do {
            buf_len =
                (DWORD)WideCharToMultiByte(CP_UTF8, 0, text, (int)_tcslen(text), m_utf8Buf, (int)m_utf8BufLen, 0,
                                           0);
...
void CJobNode::TaskError(const bool showOSRc, LPCTSTR msg, LPCTSTR severity, LPCTSTR job,
                         LPCTSTR src, LPCTSTR trg)
{
    LPTSTR eMsg = NULL;
...
void CJobNode::FatalTaskError(const bool showOSRc, LPCTSTR msg, LPCTSTR job, LPCTSTR src,
                              LPCTSTR trg)
{
    TaskError(showOSRc, msg, _T("Fatal job"), job, src, trg);
}

void CJobNode::NonFatalTaskError(const bool showOSRc, LPCTSTR msg, LPCTSTR job, LPCTSTR src,
                                 LPCTSTR trg)
{
