...
bool CTrgNode::CreateCopyFile(LPCTSTR fileName, PFILEDESC desc, DWORD desiredAccess,
                              ...
                              bool lock, int reasonBase)
{
    ...
    if (lock) {
        if (!::LockFile(desc->h, 0, 0, (DWORD)(desc->fileSize & 0xffffffff),
                        (DWORD)(desc->fileSize >> 32))) {
            ::CloseHandle(desc->h);
    ...
    const bool update)
{
    ...
    const DWORD kFromFlags = FILE_FLAG_SEQUENTIAL_SCAN | FILE_ATTRIBUTE_READONLY |
                             FILE_FLAG_BACKUP_SEMANTICS;
    const DWORD kToFlags = FILE_FLAG_SEQUENTIAL_SCAN | FILE_ATTRIBUTE_NORMAL |
                           FILE_FLAG_BACKUP_SEMANTICS;
...
CopyFileError CTrgNode::VerifyAFile(LPCTSTR fromName, LPCTSTR toName, const UINT64 fileSize,
                                    PVOID buffer,
                                    const bool fromIsShadow)
{
    ...
    const DWORD kFromFlags = FILE_FLAG_SEQUENTIAL_SCAN | FILE_ATTRIBUTE_READONLY |
                             FILE_FLAG_BACKUP_SEMANTICS;
    const DWORD kToFlags = FILE_FLAG_SEQUENTIAL_SCAN | FILE_ATTRIBUTE_READONLY |
                           FILE_FLAG_BACKUP_SEMANTICS;
    UINT64 bufferSize = CRun::kMaxThreadBufferSize;
    LPVOID buffer1 = buffer;
    LPVOID buffer2 = NULL;
