...
bool IsUserAdmin() {
    ...
    PSID AdminGroup;
    b = ::AllocateAndInitializeSid(&NtAuthority, 2, SECURITY_BUILTIN_DOMAIN_RID, DOMAIN_ALIAS_RID_ADMINS, 0, 0, 0, 0,
                                   0, 0, &AdminGroup);
    if (b) {
...
bool CopyNamedACLIfDifferent(LPCTSTR fromName, LPCTSTR toName) {
    ...
    bool protcted = false;
    do {
        if (::GetNamedSecurityInfo(fromName, SE_FILE_OBJECT, securityInfo, &pIdOwnerFrom, &pIdGroupFrom, &pDAclFrom,
                                   NULL, &pSecurityDescriptorFrom) != ERROR_SUCCESS) {
            break;
        }
    ...
    copy:
        ...
        if (securityInfo) {
            ::SetNamedSecurityInfo((LPWSTR)toName, SE_FILE_OBJECT, securityInfo, pIdOwnerFrom, pIdGroupFrom, pDAclFrom, NULL);
            updated = true;
        }
    }
    while (false);
...
tr1::wregex *ParseIncludeExcludeString(LPCTSTR names) {
    ...
    tr1::wregex *re;
    try {
        re = new tr1::wregex(CString(_T('^')) + regexp + _T('$'), regex_constants::icase | regex_constants::optimize);
    }
    catch (...) {
        re = NULL;
    }
